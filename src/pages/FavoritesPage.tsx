...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      if (!user?.preferences?.favorites?.length) {
        setLoading(false)
        return
      }

      try {
        const moviePromises = user.preferences.favorites.map(async (id) => {
  ...
  const handleClearAll = () => {
    if (window.confirm('Are you sure you want to remove all favorites?')) {
      user?.preferences?.favorites?.forEach(id => removeFromFavorites(id))
      setMovies([])
    }
  }
      ...
      <Container maxWidth="xl">
        ...
        {loading ? (
          ...
          <Box sx={{ textAlign: 'center', py: 10 }}>
            <Favorite sx={{ fontSize: 100, color: '#333', mb: 2 }} />
